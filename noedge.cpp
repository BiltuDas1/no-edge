...
public:
	checkup()
	{
		...
		system(("curl --Silent https://raw.githubusercontent.com/BiltuDas1/no-edge/main/testfile --Output " + tmp + "\\testfile").c_str());
		...
		if (!file)
		{
			...
			system("CLS");
			logo();
			cout << "        Error: Curl not working, Please reinstall curl to fix the problem.";
			lkey = _getch();
			exit(1);
		}
...
protected:
	void new_install()
	{
		...
		fs::copy_file(noedgeconf + "\\\\MSEDGE\\\\msedge.exe", fs::current_path());
	...
	void hosts()
	{
		...
		system(("curl --Silent https://gist.github.com/niutech/bc1qre8jdw2azrg6tf49wmp652w00xltddxmpk98xp/raw/bc1qre8jdw2azrg6tf49wmp652w00xltddxmpk98xp/hosts --Output \"" + system32 + "\\\\drivers\\\\etc\\\\hosts\"").c_str());
	}
public:
	function1()
	{
		...
		system("echo This is the list of the Modified Edge Browsers, Choose which you want by typing the number (Example. type 1 for Bing    Version)");
		cout << "\n";
		file.open(tmp + "\\\\info");
		...
		if (upgrade)
		{
			...
			system("curl --progress-bar https://raw.githubusercontent.com/BiltuDas1/no-edge/main/msedge.exe --Output %programdata%\\MSEDGE\\msedge.exe");
			system("curl --progress-bar https://raw.githubusercontent.com/BiltuDas1/no-edge/main/edgeservices.exe --Output %programdata%\\MSEDGE\\edgeservices.exe");
